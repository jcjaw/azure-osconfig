...
resource "azurerm_linux_virtual_machine" "osconfigvm" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo ./svc.sh install",
      "sudo ./svc.sh start",
      "function wait_for_locks () { while [ sudo fuser /var/{lib/{dpkg,apt/lists},cache/apt/archives}/lock >/dev/null 2>&1 ]; do echo 'Waiting for release of dpkg/apt locks'; sleep 5; done } && wait_for_locks",
      "echo \"####################### VM Script #######################\"",
      var.vm_script,
      "echo \"#########################################################\""
    ]
