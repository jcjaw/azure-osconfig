...
int Tpm::Get(const char* componentName, const char* objectName, MMI_JSON_STRING* payload, int* payloadSizeBytes)
{
    ...
    //     {
    //         OsConfigLogInfo(TpmLog::Get(), "TPM: got property value: '%s'", tpmProperty.c_str());
    ...
    //             if (std::regex_search(tpmProperty, match, re))
    //             {
    //                 OsConfigLogInfo(TpmLog::Get(), "TPM version from device file: %s", match[0].str().c_str());
    ...
    //     else
    //     {
    //         OsConfigLogError(TpmLog::Get(), "TPM: empty property");
    ...
    //     OsConfigLogInfo(TpmLog::Get(), "TPM %s: '%s' '%s'", objectName, data.c_str(), tpmProperty.c_str());
    // }
